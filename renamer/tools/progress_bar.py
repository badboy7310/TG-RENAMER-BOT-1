...
async def progress_bar(current, total, status_msg, start, msg):
    ...
    if round((present - start) % 3) == 0 or current == total:
        ...
        progressbar = "[{0}{1}]".format(\
            ''.join([f"{WHITE_MEDIUM_SMALL_SQUARE}" for i in range(math.floor(percentage / 10))]),
            ''.join([f"{BLACK_MEDIUM_SMALL_SQUARE}" for i in range(10 - math.floor(percentage / 10))])
            )
        current_message = f"""**{status_msg}** {round(percentage, 2)}%
{progressbar}

{BLUE_HEART} **Speed**: {humanbytes(speed)}/s

{BLUE_HEART} **Done**: {humanbytes(current)}

{BLUE_HEART} **Size**: {humanbytes(total)}

{BLUE_HEART} **Time Left**: {time_to_complete}"""
        try:
